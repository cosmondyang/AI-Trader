...
3. **封装与发布**：目录下新增 `pyproject.toml`，可在子目录内执行
   `pip install -e .` 或打包分发；对主工程没有额外依赖污染。
4. **命名约定**：新的 Python 模块与原仓库不重名（例如 `core.backtester`
...
4. **命令行入口**：
   ...
   - 支持单日或区间复盘，自动输出收益率、夏普率、最大回撤等指标，并可导出 JSON 便于可视化。
